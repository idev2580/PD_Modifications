...
from HALF_SD_MODEL import *
from HALF_SD_DATALOADER import *
...
def main():
    ...
    if(os.path.exists("HALF_SD_MODEL_CHKPNT.pth")):
        print("Load Model (CHKPNT)")
        net = load_model(net, "HALF_SD_MODEL_CHKPNT.pth")
    net = net.half()
    if(os.path.exists("HALF_SD_MODEL_CHKPNT.pth")):
        net.weight_init()

    print("WEIGHT_NAN : ", net.is_weight_nan())
    ...
    for epoch in range(50):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            #print("IN={}, PPSE={}".format(in_sure.shape, ppse.shape))
        ...
        if(epoch_loss < min_loss):
            min_loss = epoch_loss
            min_epoch = epoch
            save_name = "HALF_SD_MODEL_{}_EPOCH{}.pth".format(timestamp, epoch)
            save_model(net, save_name)
            save_model(net, "HALF_SD_MODEL_CHKPNT.pth")
            print("-> MODEL SAVED : "+save_name)

    #Save Model
    save_name = "HALF_SD_MODEL.{}.pth".format(timestamp)
    save_model(net, save_name)
